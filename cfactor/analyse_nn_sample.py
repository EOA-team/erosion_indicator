"""In-sample analysis of the NN C-factor model.

Counterpart to ``analyse_calibration_sample.py``, adapted for the outputs of
``train_cfactor_nn.py``. Same questions, same plots, same summary skeleton —
but two things differ from the β-calibration case and are handled explicitly:

1. **There is a held-out test set.** ``train_cfactor_nn.py`` writes a ``split``
   ...
   **train vs test**, and the scatter/strengths plots are drawn on the TEST
   ...
   numbers only tell you the MLP converged.
...
Inputs (defaults read from calibration_analysis_nn/, where train_cfactor_nn.py writes)
-------------------------------------------------------------------------------------
...
# ===========================================================================
CONFIG = {
    'results_dir':              'calibration_analysis_nn',
    'per_pixel_path':           'calibration_analysis_nn/nn_predictions_per_pixel.csv',
    'per_crop_path':            'calibration_analysis_nn/nn_predictions_per_crop.csv',
    'metrics_json_path':        'calibration_analysis_nn/nn_metrics.json',
...
- TRAIN vs TEST is the headline for a learned model. If train MAE is much
  lower than test MAE, the MLP memorised the training pixels — tighten
  regularisation (config['nn_params']['alpha']), shrink hidden layers, or
  get more crops. With β there was no such gap to worry about.
